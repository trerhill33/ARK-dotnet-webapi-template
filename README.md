...
- Install using the `ARK CLI` tool. Use this for release versions of the Boilerplate only.
...
After you have installed .NET, you will need to install the `ARK` console tool.

```bash
dotnet tool install --global ARK.CLI
ARK install
```

This isntall the ARK CLI tools and the associated Templates. You are now ready to create your first ARK project!

#### ARK .NET WebAPI Boilerplate
Here's how you would create a Solution using the ARK .NET WebAPI Boilerplate.
...
Run the following command. Note that, in this demonstration, I am naming my new solution as ARK.Starter.

```bash
ARK api new ARK.Starter
```

OR

```bash
ARK api n ARK.Starter
```

This will create a new .NET 7 WEBAPI solution for you using the ARK Templates.
...
```bash
dotnet tool update ARK.CLI --global
ARK update
```
...
There are also examples for mysql & mssql variations of the ARK webapi. You can find the other docker-compose files under the ./docker-compose folder. Read more about [fullstackhero's docker-compose instructions & files here](./docker-compose/README.md)
...
[![ARK dotnet webapi contributors](https://contrib.rocks/image?repo=fullstackhero/dotnet-webapi-boilerplate "ARK dotnet webapi contributors")](https://github.com/fullstackhero/dotnet-webapi-boilerplate/graphs/contributors)
